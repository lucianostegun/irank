...
function options_for_select($options = array(), $selected = '', $html_options = array())
{
  ...
  {
    $html .= content_tag('option', $value, array('value' => '')).$nl;
  }
  else if (_get_option($html_options, 'include_blank'))
  {
    $html .= content_tag('option', '', array('value' => '')).$nl;
  }
  ...
  foreach ($options as $key => $value)
  {
    ...
    {
      $html .= content_tag('optgroup', options_for_select($value, $selected, $html_options), array('label' => $key)).$nl;
    }
    else
    {
      ...
      $html .= content_tag('option', $value, $option_options).$nl;
    }
  }
...
//  $culture = _get_option($options, 'culture', $context->getUser()->getCulture());
  ...
  if (true || $calendar_button_img = _get_option($options, 'calendar_button_img'))
  {
...
function button_tag( $buttonId, $text, $options=array() ){
	...
	if( $image )
		$image = image_tag('button/'.($disabled?'disabled/':'').$image, array('id'=>$buttonId.'Image', 'align'=>'absmiddle', 'style'=>'margin-top: -2px; margin-right: 5px'));
...
function snip_tag( $fieldName, $fieldValue, $options=array() ){

	$nl            = chr(10);
	$htmlOptions   = array('size'=>2, 'readonly'=>true, 'style'=>'height: 18px');
	$options       = array_merge($options, $htmlOptions);
	$id            = $options['id'];
	$checkFunction = array_key_exists('checkFunction', $options)?$options['checkFunction']:'function(){return true;}';
	$fieldValue    = sprintf('%03d', $fieldValue);
	
	$html  = '<div style="clear: both">'.$nl;
	$html .= '	<div style="float: left">'.$nl;
	$html .= '		'.input_tag($fieldName, $fieldValue, $options).$nl;
	$html .= '	</div>'.$nl;
	$html .= '	<div style="float: left; margin-top: 1px !ie">'.$nl;
	$html .= '		<div><div onclick="incraseSnip(\''.$id.'\', '.$checkFunction.')" onmousedown="changeSnipImage(this, false)" onmouseup="changeSnipImage(this, true)" class="snipButton" style="background: url(\'/images/snipUp.png\')">'.image_tag('blank.gif').'</div></div>'.$nl;
	$html .= '		<div><div onclick="decraseSnip(\''.$id.'\', '.$checkFunction.')" onmousedown="changeSnipImage(this, false)" onmouseup="changeSnipImage(this, true)" class="snipButton" style="background: url(\'/images/snipDown.png\')">'.image_tag('blank.gif').'</div></div>'.$nl;
	$html .= '	</div>'.$nl;
	$html .= '</div>';
 ...
 * em progresso, utilizado nos formulários de cadastro 
 *
 * @author     Luciano Stegun
 * @param      String: ID do indicador para o caso de mais de uma janela renderizada na mesma página
 */
function getLoading( $indicatorId=null ){
	
	echo get_partial( 'home/include/loading', array('indicatorId'=>$indicatorId) );
}

/**
 * Método que inclui um indicador visual de informação de processamento
 * em progresso, utilizado nas janelas que utilizam processamento para salvar informações 
 *
 * @author     Luciano Stegun
 * @param      String: ID do indicador para o caso de mais de uma janela renderizada na mesma página
 */
...
/**
 * Método que inclui um indicador visual de informação de processamento realizado com sucesso
 *
 * @author     Luciano Stegun
 * @param      String: ID do indicador para o caso de mais de uma janela renderizada na mesma página
 */
