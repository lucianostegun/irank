...
<tr onmouseover="this.className='recordRowOver'" onmouseout="this.className=''">
	...
	<td onclick="<?php echo $link ?>" class="icon" align="left"><?php echo ($comments>0?image_tag('icon/comment', array('title'=>'Este evento possui '.$comments.' comentário'.($comments==1?'':'s'))):'') ?></td>
	<td onclick="<?php echo $link ?>" class="icon" align="left"><?php echo ($photos>0?image_tag('icon/photo', array('title'=>'Este evento possui '.$photos.' foto'.($photos==1?'':'s'))):'') ?></td>
</tr>
<?php
	endforeach;
