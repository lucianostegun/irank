...
REDIPS.drag = (function() {
	var init, init_tables, enable_drag, enable_table, img_onmousemove, handler_onmousedown, handler_ondblclick, table_top, handler_onmouseup, handler_onmousemove, element_drop, element_deleted, cell_changed, handler_onresize, set_trc, set_position, setTdStyle, getTdStyle, box_offset, calculate_cells, getScrollPosition, autoscrollX, autoscrollY, clone_div, copy_properties, clone_limit, elementControl, get_style, find_parent, find_cell, save_content, relocate, empty_cell, shift_cells, move_object, delete_object, animation, get_table_index, get_position, row_opacity, row_empty, row_clone, row_drop, form_elements, normalize, has_childs, obj_margin = null, window_width = 0, window_height = 0, scroll_width = null, scroll_height = null, edge = {
	...
	}, scroll_object, bgstyle_old, scrollable_container = [], tables = [], sort_idx, moved, cloned, cloned_id = [], currentCell = [], div_drag = null, div_box = null, pointer = {
		x : 0,
		y : 0
	}, threshold = {
		x : 0,
		...
		value : 7,
		flag : false
	}, shift_key = false, clone_class = false, table = null, table_old = null, table_source = null, row = null, row_old = null, row_source = null, cell = null, cell_old = null, cell_source = null, obj = false, obj_old = false, mode = 'cell', hover = {
		color_td : '#E7AB83',
		color_tr : '#E7AB83'
	}, bound = 25, speed = 20, only = {
		div : [],
		cname : 'only',
		other : 'deny'
	}, mark = {
	...
	init_tables = function() {
		...
		for (i = 0, j = 0; i < tables_nodeList.length; i++) {
			if (tables_nodeList[i].parentNode.id === 'redips_clone'
					|| tables_nodeList[i].className.indexOf('nolayout') > -1) {
				continue;
			}
		...
		for (i = 0, group_idx = sort_idx = 1; i < tables.length; i++) {
			if (tables[i].redips.nestedLevel === 0) {
				...
				for (j = 0; j < nested_tables.length; j++) {
					...
					nested_tables[j].redips.sort = sort_idx * 100
							+ nested_tables[j].redips.nestedLevel;
				}
				group_idx++;
				sort_idx++;
			}
	...
	handler_onmousedown = function(e) {
		...
		obj_margin = [ Y - offset[0], offset[1] - X, offset[2] - Y,
				X - offset[3] ];
	...
	row_drop = function(r_table, r_row, table_mini) {
		var tbl = tables[r_table], ts = tbl.rows[0].parentNode, animated = false, tr, rp, src, rowIndex, delete_srow, drop;
		...
		if (drop !== false) {
			if (!animated
					&& target_cell.className.indexOf(REDIPS.drag.trash_cname) > -1) {
				if (cloned) {
	...
	handler_onmouseup = function(e) {
		var evt = e || window.event, target_table, r_table, r_row, mt_tr, X, Y, i, target_elements, target_elements_length;
		X = evt.clientX;
		Y = evt.clientY;
		...
		if (cloned && mode === 'cell'
			...
			if (table < tables.length) {
				...
				r_table = table;
				r_row = row;
			} else if (table_old === null || row_old === null
					|| cell_old === null) {
			...
			if (mode === 'row') {
				...
				REDIPS.drag.myhandler_notmoved();
			} else if (cloned && table_source === table && row_source === row
					&& cell_source === cell) {
				obj.parentNode.removeChild(obj);
				cloned_id[obj_old.id] -= 1;
				REDIPS.drag.myhandler_notcloned();
					...
					&& (X < target_table.redips.offset[3]
							|| X > target_table.redips.offset[1]
	...
	element_drop = function() {
		...
		if (drop !== false) {
			...
			if (REDIPS.drag.multiple_drop === 'top'
					&& target_cell.hasChildNodes()) {
	...
	handler_onmousemove = function(e) {
		var evt = e || window.event, bound = REDIPS.drag.bound, sca, X, Y, deltaX, deltaY, i, scrollPosition;
		if (evt.touches) {
			X = pointer.x = evt.touches[0].clientX;
			Y = pointer.y = evt.touches[0].clientY;
		} else {
		...
		if (!moved) {
			if (mode === 'cell'
					&& (clone_class || (REDIPS.drag.clone_shiftKey === true && shift_key))) {
				...
				if (mode === 'row') {
					if (clone_class
							|| (REDIPS.drag.clone_shiftKey_row === true && shift_key)) {
						cloned = true;
					}
					REDIPS.drag.obj_old = obj_old = obj;
					REDIPS.drag.obj = obj = row_clone(obj);
			...
			if (X > window_width - obj_margin[1]) {
				obj.style.left = (window_width - (obj_margin[1] + obj_margin[3]))
						+ 'px';
			}
			if (Y > window_height - obj_margin[2]) {
				obj.style.top = (window_height - (obj_margin[0] + obj_margin[2]))
						+ 'px';
			}
		}
		moved = true;
		...
		if (X < div_box[1]
				...
				&& edge.flag.x === 0
				&& edge.flag.y === 0
				&& (currentCell.containTable || (X < currentCell[3]
						|| X > currentCell[1] || Y < currentCell[0] || Y > currentCell[2]))) {
			set_trc();
			cell_changed();
		}
		edge.page.x = bound
				- (window_width / 2 > X ? X - obj_margin[3] : window_width - X
						- obj_margin[1]);
		if (edge.page.x > 0) {
		...
		edge.page.y = bound
				- (window_height / 2 > Y ? Y - obj_margin[0] : window_height
						- Y - obj_margin[2]);
		if (edge.page.y > 0) {
		...
		for (i = 0; i < scrollable_container.length; i++) {
			...
			if (sca.autoscroll && X < sca.offset[1] && X > sca.offset[3]
				...
				edge.div.x = bound
						- (sca.midstX > X ? X - obj_margin[3] - sca.offset[3]
								: sca.offset[1] - X - obj_margin[1]);
				if (edge.div.x > 0) {
					if (edge.div.x > bound) {
						edge.div.x = bound;
					}
				...
				edge.div.y = bound
						- (sca.midstY > Y ? Y - obj_margin[0] - sca.offset[0]
								: sca.offset[2] - Y - obj_margin[2]);
				if (edge.div.y > 0) {
					if (edge.div.y > bound) {
						edge.div.y = bound;
					}
	...
	set_trc = function() {
		var previous, cell_current, row_offset, row_found, cells, empty, mark_found, only_found, single_cell, tos = [], X, Y, i;
		previous = function() {
			if (table_old !== null && row_old !== null && cell_old !== null) {
				table = table_old;
				row = row_old;
		...
		for (table = 0; table < tables.length; table++) {
			...
			if (tables[table].sca !== undefined) {
				tos[0] = tos[0] > tables[table].sca.offset[0] ? tos[0]
						: tables[table].sca.offset[0];
				tos[1] = tos[1] < tables[table].sca.offset[1] ? tos[1]
						: tables[table].sca.offset[1];
				tos[2] = tos[2] < tables[table].sca.offset[2] ? tos[2]
						: tables[table].sca.offset[2];
				tos[3] = tos[3] > tables[table].sca.offset[3] ? tos[3]
						: tables[table].sca.offset[3];
			}
			if (tos[3] < X && X < tos[1] && tos[0] < Y && Y < tos[2]) {
				...
				} while (tables[table].redips.rowspan && cell === -1
						&& row-- > 0);
				if (row < 0 || cell < 0) {
					previous();
				} else if (row !== row_found) {
					currentCell[0] = row_offset[row][0];
					currentCell[2] = currentCell[0]
							+ tables[table].rows[row].cells[cell].offsetHeight;
				...
				if (cell_current.childNodes.length > 0
						&& cell_current.getElementsByTagName('table').length > 0) {
					currentCell.containTable = true;
				} else {
					currentCell.containTable = false;
				}
				if (cell_current.className.indexOf(REDIPS.drag.trash_cname) === -1) {
					only_found = cell_current.className
							.indexOf(REDIPS.drag.only.cname) > -1 ? true
							: false;
					if (only_found === true) {
					...
					} else if (only.div[obj.id] !== undefined
							&& only.other === 'deny') {
						previous();
						break;
					} else {
						mark_found = cell_current.className
								.indexOf(REDIPS.drag.mark.cname) > -1 ? true
								: false;
						if ((mark_found === true && REDIPS.drag.mark.action === 'deny')
								|| (mark_found === false && REDIPS.drag.mark.action === 'allow')) {
							if (cell_current.className
									.indexOf(mark.exception[obj.id]) === -1) {
								previous();
								break;
							}
						}
				...
				single_cell = cell_current.className.indexOf('single') > -1 ? true
						: false;
				if (mode === 'cell') {
					if ((REDIPS.drag.drop_option === 'single' || single_cell)
						...
						if (!empty && table_old !== null && row_old !== null
								&& cell_old !== null) {
							if (table_source !== table || row_source !== row
									|| cell_source !== cell) {
								previous();
								break;
							}
					...
					if (cell_current.parentNode.redips
							&& cell_current.parentNode.redips.empty_row) {
						previous();
						break;
					}
				}
	...
	set_position = function() {
		if (table < tables.length && table !== null && row !== null
				&& cell !== null) {
			bgstyle_old = getTdStyle(table, row, cell);
	...
	setTdStyle = function(ti, ri, ci, t) {
		...
		if (mode === 'cell' && threshold.flag) {
			...
			s.backgroundColor = (t === undefined) ? REDIPS.drag.hover.color_td
					: t.color[0].toString();
	...
	getTdStyle = function(ti, ri, ci) {
		var tr, i, c, t = {
			...
			var width = 'border' + name + 'Width', style = 'border' + name
					+ 'Style', color = 'border' + name + 'Color';
			return [ get_style(c, width), get_style(c, style),
					get_style(c, color) ];
		};
		if (mode === 'cell') {
	...
	box_offset = function(box, position, box_scroll) {
		...
		return [ oTop, oLeft + box_old.offsetWidth,
				oTop + box_old.offsetHeight, oLeft ];
	};
	...
	autoscrollX = function(so) {
		...
		if (edge.flag.x > 0) {
			calculate_cells();
			set_trc();
			if (X < div_box[1] && X > div_box[3] && Y < div_box[2]
					&& Y > div_box[0]) {
				cell_changed();
			}
		}
		...
		if (edge.flag.x > 0
				&& ((edgeCrossed < 0 && scrollPosition > 0) || (edgeCrossed > 0 && scrollPosition < maxsp))) {
			if (scroll_object === window) {
	...
	enable_drag = function(enable_flag, el, type) {
		var i, j, k, divs = [], tbls = [], borderStyle, opacity, cursor, overflow, autoscroll, enabled, cb, handler1, handler2, position, regex_drag = /\bdrag\b/i, regex_noautoscroll = /\bnoautoscroll\b/i;
		...
		for (i = 0, j = 0; i < divs.length; i++) {
			if (regex_drag.test(divs[i].className)) {
				...
				} else if (enable_flag === true
						&& typeof (opacity) === 'number') {
					divs[i].style.opacity = '';
					divs[i].style.filter = '';
				} else if (enable_flag === false
						&& typeof (opacity) === 'number') {
	...
	find_cell = function(param, el) {
		var tbl = find_parent('TABLE', el), ri, ci, c;
		switch (param) {
		case 'firstInColumn':
			ri = 0;
			ci = el.cellIndex;
	...
	save_content = function(tbl, type) {
		var query = '', tbl_start, tbl_end, tbl_rows, cells, tbl_cell, cn, id, r, c, d, JSONobj = [], pname = REDIPS.drag.save_pname;
		if (typeof (tbl) === 'string') {
			tbl = document.getElementById(tbl);
		}
		if (tbl !== undefined && typeof (tbl) === 'object'
				&& tbl.nodeName === 'TABLE') {
				...
				for (c = 0; c < cells; c++) {
					...
					if (tbl_cell.childNodes.length > 0) {
						for (d = 0; d < tbl_cell.childNodes.length; d++) {
							...
							if (cn.nodeName === 'DIV'
									&& cn.className.indexOf('drag') > -1) {
								query += pname + '[]=' + cn.id + '_' + r + '_'
										+ c + '&';
		...
		if (mode === 'animation') {
			...
			for (i = 0; i < cn; i++) {
				if (from.childNodes[i].nodeType === 1
						&& from.childNodes[i].nodeName === 'DIV'
						&& REDIPS.drag.obj !== from.childNodes[i]) {
					an_counter++;
			...
			for (i = 0, j = 0; i < cn; i++) {
				if (from.childNodes[j].nodeType === 1
						&& from.childNodes[j].nodeName === 'DIV'
						&& REDIPS.drag.obj !== from.childNodes[j]) {
	...
	shift_cells = function(td1, td2) {
		...
		switch (soption) {
		case 'vertical2':
			pos = (tbl1 === tbl2 && td1.cellIndex === td2.cellIndex) ? pos1
					: find_cell('lastInColumn', td2);
			break;
		case 'horizontal2':
			pos = (tbl1 === tbl2 && td1.parentNode.rowIndex === td2.parentNode.rowIndex) ? pos1
					: find_cell('lastInRow', td2);
			break;
		default:
	...
	animation = function(i, p) {
		...
		if ((i < p.last && p.direction > 0)
				|| ((i > p.last) && p.direction < 0)) {
			setTimeout(function() {
				animation(i, p);
				...
				if (opacity === 'empty') {
					...
					for (j = 0; j < td[i].childNodes.length; j++) {
						if (td[i].childNodes[j].nodeType === 1) {
							...
							td[i].childNodes[j].style.filter = 'alpha(opacity='
									+ opacity + ')';
						}
					}
				}
