...
var globalActiveDHTMLGridObject;String.prototype._dhx_trim=function(){return this.replace(/&nbsp;/g, " ").replace(/(^[ \t]*)|([ \t]*$)/g, "")};function dhtmlxArray(ar){return dhtmlXHeir((ar||new Array()), dhtmlxArray._master)};dhtmlxArray._master={_dhx_find:function(pattern){for (var i = 0;i < this.length;i++){if (pattern == this[i])return i};return -1},
 ...
 this.cellWidthPC[ind]=pcWidth};if (this.sortImg.style.display!="none")this.setSortImgPos()}};
 ...
 };this.setFooterLabel=function(c, label, ind){return this.setColumnLabel(c,label,ind,this.ftr)};
 
 this.setColumnLabel=function(c, label, ind, hdr){
	 var z = (hdr||this.hdr).rows[ind||1];
	 var col = (z._childIndexes ? z._childIndexes[c] : c);
	 if (!z.cells[col])return;if (!this.useImagesInHeader){var hdrHTML = "<div class='hdrcell'>"
 ...
 this.clearAll=function(header){if (!this.obj.rows[0])return;if (this._h2){this._h2=new dhtmlxHierarchy();if (this._fake){if (this._realfake)this._h2=this._fake._h2;else
 ...
 this.UserData=[]
 this.selectedRows=dhtmlxArray();if (this.pagingOn || this._srnd)this.xmlFileUrl="";if (this.pagingOn)this.changePage(1);if (this._contextCallTimer)window.clearTimeout(this._contextCallTimer);if (this._sst)this.enableStableSorting(true);this._fillers=this.undefined;
 //this.setSortImgState(false);
 this.setSizes();this.callEvent("onClearAll", [])};this.sortField=function(ind, repeatFl, r_el){if (this.getRowsNum()== 0)
...
// Código alterado Luciano Stegun
 ...
 parse:function(data, call, type){
	 ...
	 if (!this._contextCallTimer && call!=null )
 ...
 function updateGridbox( gridboxObj, xmlString, mode ){

		gridboxObj.clearAll();
		gridboxObj.loadXMLString( xmlString );
		
		if( mode )
			gridboxObj.setSortImgState(true, 0, mode, 0);

		hideIndicator();
	}
		
	 function startLoadingGridbox( gridboxObj ){

		var columnCount = gridboxObj.getColumnCount();
		gridboxObj.clearAll();
		
		var xmlString = '<?xml version="1.0" encoding="UTF-8"?><rows><row id="100000000"><cell colspan="'+columnCount+'">&lt;img src="/images/ajaxLoader.gif" align="absmiddle" style="margin-right: 5px" /&gt;Carregando, aguarde...</cell><cell></cell><cell></cell><cell></cell></row></rows>';
		gridboxObj.loadXMLString(xmlString, null);
	 }

	 function checkGridboxSelection( gridboxObj, rowId ){
	 ...
	 var checkIsHeaderFilter = function( colId, sortType ){

	 	return _SortingEnabled;
	 }
	 
	 function adjustGridboxHeight(divId, searchOptions){
		...
		var height = getScreenHeight()-difference;

		$(divId).style.height = height;
	} 
