...
class MyHorBar extends pChart
  {
   ...
   function drawHorStackedBarGraph($Data,$DataDescription,$Alpha=50,$Contiguous=FALSE)
    {
     /* Validate the Data and DataDescription array */
     $this->validateDataDescription("drawBarGraph",$DataDescription);
     $this->validateData("drawBarGraph",$Data);
           ...
           if ( is_numeric($Data[$Key][$ColName]) )
            {
			 ...
			 $X1 = $XPos+1;
			 $X2 = $XPos+$SeriesWidth-1;
			 $Y1 = $YBottom;
			 $Y2 = $YPos;
			 
			 $rows   = count($Data);
			 $height = $this->GArea_Y2-$this->GArea_Y1;
			 
			 $incrase = ($rows<=6?15:7);
			 $X1 = $this->GArea_X1;
			 $X2 = $this->GArea_X1+($Y1-$Y2);
			 $Y1 = $this->GArea_Y1+$incrase+($height/$rows*$Key);
			 $Y2 = $Y1+$SeriesWidth-1;
   ...
   function drawHorScale($Data,$DataDescription,$ScaleMode,$R,$G,$B,$DrawTicks=TRUE,$Angle=0,$Decimals=1,$WithMargin=FALSE,$SkipLabels=1,$RightScale=FALSE)
	...
	$this->drawLine($XPos,$this->GArea_Y1,$XPos,$this->GArea_Y1-5,$R,$G,$B);
       ...
       $TextWidth = $Position[2]-$Position[0];
		 $decrase = 0;//($i%2==0?15:0);
         imagettftext($this->Picture,$this->FontSize,0,$XPos-($this->FontSize/2),($decrase*-1)+$this->GArea_Y1-10,$C_TextColor,$this->FontName,$Value);
...
/* Compute and draw the scale */
   ...
   function writeHorValues($Data,$DataDescription,$Series)
    {
     /* Validate the Data and DataDescription array */
     $this->validateDataDescription("drawBarGraph",$DataDescription);
     $this->validateData("drawBarGraph",$Data);
           ...
           if ( is_numeric($Data[$Key][$ColName]) )
            {
			 ...
			 $incrase = ($rows<=6?15:7);
			 $X2 = $this->GArea_X1+($Y1-$Y2)+10;
			 $Y1 = $this->GArea_Y1+$incrase+($height/$rows*$Key)+($SeriesWidth/1.8);
			 $Y2 = $Y1+$SeriesWidth-1;
			 
             $C_TextColor =$this->AllocateColor($this->Picture,0,0,0);
             imagettftext($this->Picture,$this->FontSize,0,$X2,$Y1,$C_TextColor,$this->FontName,$Value);
            }
          }
         $XPos = $XPos + $this->DivisionWidth;
        }
...
//			 
			 $incrase = ($rows<=6?15:7);
//			 $X1 = $this->GArea_X1;
//			 $XOffset = $this->GArea_X1+($Y1-$Y2);
			 $YOffset = $this->GArea_Y1+$incrase+($height/$rows*$Key)+($SeriesWidth/2);
//			 $Y2 = $Y1+$SeriesWidth-1;
