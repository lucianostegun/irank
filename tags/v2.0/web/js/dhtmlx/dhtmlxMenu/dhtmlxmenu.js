...
function dhtmlXMenuObject(baseId, skin) {
		...
		if ((this.idPull[pId] != null) && (this.idPull[id] != null)) {
			...
			if ((this.sxDacProc != null)
				...
				if (this._isIE6) {
					...
					this.idPull[pIdIE6].style.width = this.idPull[pId].offsetWidth
							+ "px";
					this.idPull[pIdIE6].style.height = this.idPull[pId].offsetHeight
							+ "px";
	...
	this._redistribSubLevelSelection = function(id, parentId) {
		...
		if (this.itemPull[id]["state"] == "enabled") {
			...
			if (this.itemPull[id]["complex"] && this.dLoad
				...
				xmlLoader.loadXML(this.dLoadUrl + this.dLoadSign
	...
	this._doOnClick = function(id, type, casState) {
		...
		if (this.itemPull[this.idPrefix + id]["href_link"] != null) {
			alert(this.itemPull[this.idPrefix + id]["href_link"]);
		...
		// Exceção para o item HOME (menu1)
		if (type.charAt(0) == "c" && id!='menu1' )
			return;
		if (type.charAt(1) == "d")
			return;
		if (type.charAt(2) == "s")
	...
	m.onmouseover = function() {
		...
		if (main_self.itemPull[this.id]["state"] == "enabled") {
			...
			if (main_self.dLoad
				...
				;
				var xmlLoader = new dtmlXMLLoaderObject(main_self._xmlParser,
						window);
				main_self.itemPull[this.id]["loaded"] = "get";
				main_self.callEvent("onXLS", []);
				xmlLoader.loadXML(main_self.dLoadUrl + main_self.dLoadSign
						+ "action=loadMenu&parentId="
...
dhtmlXMenuObject.prototype.addContextZone = function(zoneId) {
	...
	for ( var a in this.contextZones) {
		zoneExists = zoneExists || (a == zoneId)
				|| (this.contextZones[a] == zone)
	}
	;
	if (zoneExists == true)
		return false;
...
dhtmlXMenuObject.prototype._renderSublevelItem = function(id, pos) {
	...
	k.onclick = function(e) {
		if (!main_self.checkEvent("onClick")
				&& main_self.itemPull[this.id]["complex"]) {
			return
		}
		;
		e = e || event;
...
dhtmlXMenuObject.prototype._renderSeparator = function(id, pos) {
	var level = (this.context ? "SubLevelArea"
