...
class iCal {
	...
	*/
	var $prodid = '-//irank.com.br//iCal Class MIMEDIR//EN';
	...
	*/
	function iCal($prodid = '', $method = 1, $downloaddir = '') {
		...
		$this->events_filename  = (string) microtime() . '.ics';
	} // end constructor
				...
				if (count($attendees) > 0) {
					foreach ($attendees as $name => $data) {
						...
						if (strlen(trim($email)) > 5) {
							$role     = (int) $values[1];
							$partStat = (string) $values[2];
							$output .= (string) "ATTENDEE;ROLE=" . $this->getAttendeeRole($role) . ";PARTSTAT=".$partStat.";RSVP=TRUE;CN=" . $name . ":MAILTO:" . $email . "\r\n";
						} // end if
					} // end foreach
				} // end if
			} elseif ($format === 'xcs') {
...
//					$this->output .= (string) "LOCATION" . $event->getLanguage() . ";ENCODING=QUOTED-PRINTABLE:" . $this->quotedPrintableEncode($event->getLocation()) . "\r\n";
					$this->output .= (string) "LOCATION" . $event->getLanguage() . ";ENCODING=QUOTED-PRINTABLE:" . $event->getLocation() . "\r\n";
				} // end if
...
//					$this->output .= (string) "DESCRIPTION" . $event->getLanguage() . ";ENCODING=QUOTED-PRINTABLE:" . str_replace('\n', '=0D=0A=',str_replace('\r', '=0D=0A=', $this->quotedPrintableEncode($event->getDescription()))) . "\r\n";
					$this->output .= (string) "DESCRIPTION" . $event->getLanguage() . ";ENCODING=QUOTED-PRINTABLE:" . $event->getDescription() . "\r\n";
				} // end if
				$this->output .= (string) "SUMMARY" . $event->getLanguage() . ";ENCODING=QUOTED-PRINTABLE:" . $event->getSummary() . "\r\n";
	...
	*/
	function getFilePath() {
		...
		return (string) $this->download_dir . DIRECTORY_SEPARATOR . $this->events_filename;
	} // end function
	/**#@-*/
