    	...
    	<ul>
            <?php
            	if( array_key_exists('new', $toolbarList) || in_array('new', $toolbarList) )
            		echo '<li>'.link_to(image_tag('backend/icons/control/16/plus').'<span>Novo</span>', '#doGetNew()').'</li>';
            	
            	if( array_key_exists('save', $toolbarList) || in_array('save', $toolbarList) )
					echo '<li>'.link_to(image_tag('backend/icons/control/16/save').'<span>Salvar</span>', '#doSaveMain()').'</li>';
            	...
            	if( array_key_exists('delete', $toolbarList) || in_array('delete', $toolbarList) ){
					...
					echo '<li>'.link_to(image_tag('backend/icons/control/16/delete').'<span>Excluir</span>', $executeAction).'</li>';
            	}
            	...
            	foreach($toolbarList as $toolbar){
            		...
            		echo '<li>'.link_to(image_tag('backend/icons/control/16/'.$toolbar['image']).'<span>'.$toolbar['label'].'</span>', $toolbar['url']).'</li>';
            	}
			?>
        </ul>
    </div>
